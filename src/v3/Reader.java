...
public class Reader {
	...
	protected ArrayList<Commercial> getCommercials() {
		...
		while (reader.hasNextLine()) {
			...
			Commercial c = new Commercial(getNextVar());
			coms.add(c);
		}
	...
	protected ArrayList<RadioSpot> getSpots() {
		...
		while (reader.hasNextLine()) {
			...
			RadioSpot s = new RadioSpot(getNextVar());
			spots.add(s);
		}
		return spots;
	}
	...
	private String getNextVar() {
		...
		do {
			if(testing) System.out.println("lineReader: "+lineReader.toString());

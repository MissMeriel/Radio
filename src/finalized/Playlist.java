...
public class Playlist implements Serializable {
	...
	public Playlist(String playlistName) {
		...
		/*
		 * try { adRdr = new Reader("src/AdList.txt"); } catch
		 * (FileNotFoundException e) { e.printStackTrace(); } adList =
		 * adRdr.getCommercials(); try { spotRdr = new
		 * Reader("src/RadioSpotList.txt"); } catch (FileNotFoundException e) {
		 * e.printStackTrace(); } radioSpotList = spotRdr.getSpots(); try {
		 * newsRdr = new Reader("src/NewsReportList.txt"); } catch
		 * (FileNotFoundException e) { e.printStackTrace(); } newsReportList =
		 * newsRdr.getReports(false); try { trafficRdr = new
		 * Reader("src/TrafficReportList.txt"); } catch (FileNotFoundException
		 * e) { e.printStackTrace(); } trafficReportList =
		 * trafficRdr.getReports(true);
		 */
	}
	...
	public void addItem(Broadcastable s) throws OverTimeException,
		...
		if (testing)
			...
			if (s.getLength().compareTo(timeLeft()) <= 0) {
				...
				// if it's been a half hour since adding Promotions & there's
				// room
				// for them
				if (time >= promotionTime && time <= (4 * 3600 - 600)) {
					...
					for (int k = 0; k < 2; k++) {
						playlist.add(radioSpotList.get(rand
								.nextInt(radioSpotList.size())));
						currentLength = currentLength
				...
				// if it's been an hour since adding Reports & there's room for
				// them
	...
	private void adjustAdsAndReports() {
		...
		Iterator<Broadcastable> iter2 = playlist.iterator();
		while (iter2.hasNext()) {
			...
			if (alLength.getSeconds() >= promoTime
				...
				// if it's been an hour since adding Reports & there's time
				if (alLength.getSeconds() >= reportTime
						&& alLength.getSeconds() <= (4 * 3600 - 300)) {

					al.add(newsReportList.get(rand.nextInt(newsReportList
							.size())));
	...
	public void setAdList(ArrayList<Commercial> comms) {
		adList = comms;
	}

	public void setTrafficReportList(ArrayList<Report> t) {
		trafficReportList = t;
	}

	public void setNewsReportList(ArrayList<Report> news) {
		newsReportList = news;
	}

	public void setRadioSpotList(ArrayList<RadioSpot> spots) {
		radioSpotList = spots;
	}
